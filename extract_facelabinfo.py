...
def save_debug_image(bgr_image: np.ndarray, detection_res: List[DetectionInfo], debug_path: str) -> None:
    ...
    for i, detection in enumerate(detection_res, 1):
        ...
        ax.add_patch(bbox)
        ax.scatter(
            detection.source_landmarks[:, 0], detection.source_landmarks[:, 1], alpha=0.5, marker="x")
        ...
        axes[i].scatter(detection.aligned_landmarks[:, 0],
                        detection.aligned_landmarks[:, 1], alpha=0.5, marker="x")
        axes[i].set_axis_off()
